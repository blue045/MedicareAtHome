...
export function rowToHospital(row) {
  ...
  return {
    ...
    doctorIds: safeJsonParse(row.doctorIds, []),
    address: row.address || "",
    phone: row.phone || "",
    whatsapp: row.whatsapp || "",
...
export function rowToSettings(row) {
  ...
  return {
    ...
    pageContent: safeJsonParse(row.pageContent, {}),
...
function hospitalRecord(item = {}) {
  return {
    ...
    doctorIds: JSON.stringify(Array.isArray(item.doctorIds) ? item.doctorIds.map((id) => String(id || "").trim()).filter(Boolean) : []),
...
function settingsRecord(settings) {
  return {
    ...
    pageContent: JSON.stringify(settings.pageContent || {}),
